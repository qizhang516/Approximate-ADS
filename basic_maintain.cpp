...
struct Edge { type t1, t2, to; bool exists, initiated; };
...
struct Graph {
	...
	inline void init_adj_edges(type u); bool* have_initiated;
	...
	void BasicDelete(type eid);
	void BasicInsert(type eid);
	...
	~Graph() {
		free(e); free(d); free(adj_length); free(cur); free(dist); free(p); free(have_initiated); free(undeg);
		for(int i=0; i<n; i++){
				free(adj[i]);
		}
			free(adj);
...
void Graph::read_edge(char* address) {
	...
	while (fgets(line, 200, file)) {
		...
		e[m].to = -1; e[m].exists = true; e[m].initiated = false;
		m++;
	...
	have_initiated = (bool*)malloc(n * sizeof(bool)); memset(have_initiated, 0, n * sizeof(bool));
...
void Graph::get_densest() {
	...
	for (type i = 0; i < R.size; i++) {
		type u = R.nodes[i];
		volR += undeg[u];
		init_adj_edges(u); allnodes.insert(u);
...
void Graph::generateUpdateEdges(type updatenum) {
	while (totalupdateedges.size() < updatenum) {
		type ueid = (rand() % (m));
		if (ueid % 2 == 1)
			ueid--;
		totalupdateedges.insert(ueid);
	}
}

inline void Graph::init_adj_edges(type u) {
	if (have_initiated[u])
		return;
	have_initiated[u] = true;
	for (type j = 0; j < adj_length[u]; j++) {
		if (!e[adj[u][j]].initiated) e[adj[u][j]].to = e[adj[u][j]].t1, e[adj[u][j]].initiated = true;
	}
}
...
void Graph::ReTest(type test_value_) {
	...
	for (type i = 0; i < allnodes.size; i++) {
		...
		if (d[u] >= pivot)
			init_adj_edges(u), nownodes.insert(u);
	}
...
bool Graph::DinicBFS() {
	...
	s_node.clear();
	for (type i = 0; i < nownodes.size; i++) if (d[nownodes.nodes[i]] > pivot) s_node.insert(nownodes.nodes[i]);
...
void Graph::BasicDelete(type eid) {
	type u = e[eid].t1, v = e[eid].t2;

	undeg[u]--, undeg[v]--; if (!R.in[u]) d[u]++; if (!R.in[v]) d[v]++;

	e[eid].exists = false;
	e[eid + 1].exists = false;

	if (e[eid].initiated) d[e[eid].to]--;
	else d[e[eid].t1]--;
	if (e[eid + 1].initiated) d[e[eid + 1].to]--;
	else d[e[eid + 1].t1]--;

	ReTest(alpha);
	if (density_ge_test_value()) {
		ReTest(alpha + 1);
		if (density_ge_test_value()) {
			alpha++;
		}
...
void Graph::BasicInsert(type eid) {
	type u = e[eid].t1, v = e[eid].t2;
	...
	e[eid].initiated = true, e[eid].to = e[eid].t1, d[e[eid].t1]++;
	e[eid + 1].initiated = true, e[eid + 1].to = e[eid + 1].t1, d[e[eid + 1].t1]++;
...
int main(int argc, char** argv) {
	char adjacent_list_address[100], R_set_address[100], A_set_address[100], updateedge_address[100];
	if (argc == 5) {
		strcpy(adjacent_list_address, argv[1]); strcpy(R_set_address, argv[2]); strcpy(A_set_address, argv[3]); strcpy(updateedge_address, argv[4]);
	}
	else {
		printf("Usage:\n");
		printf("./main adjacent_list_address R_set_address A_set_address updateedge_address\n");
		return 0;
	}
	double runtime;

	FILE* updatefile = fopen(updateedge_address, "w");
	...
	for (type i = 0; i < RSet.size(); i++) {
		...
		realcnt++;
		// G->generateUpdateEdges(10000);
		printf("---query %d update---\n", i);
		for (type eid : totalupdateedges) {
			// printf("----------------\nNow update edge (%d, %d)\n", G->e[eid].t1, G->e[eid].t2);
			timer_start();
			G->BasicDelete(eid);
			runtime = timer_end();
