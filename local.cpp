...
struct Graph {
	...
	~Graph() {
		free(e); free(d); free(adj_length); free(cur); free(dist); free(p); free(have_initiated);
		for(int i=0; i<n; i++){
				free(adj[i]);
			}
			free(adj);
...
void read_RSet(char* address, type num) {
	...
	for (int i = 0; i < querytimes; i++) {
		...
		// std::cout<<"output R"<<i<<std::endl;
		// for(int j=0; j<RSet[i].size; j++){
		// 	std::cout<<RSet[i].nodes[j]<<"\t";
		// }
		// std::cout<<"\n";
...
void Graph::get_densest() {
	...
	for (type i = 0; i < R.size; i++) {
		...
		if (A.in[u]) d[u] = adj_length[u] / 2 + INF;
		else d[u] = adj_length[u] / 2;
	}
...
inline void Graph::init_adj_edges(type u) {
	...
	for (type j = 0; j < adj_length[u]; j++) {
		if (e[adj[u][j]].to == -1) e[adj[u][j]].to = e[adj[u][j]].t1;
	}
}
type last_test_value = INF;
void Graph::ReTest(type test_value_) {
	...
	if (test_value < last_test_value) {
		...
		for (type i = 0; i < nownodes.size; i++) {
			if (d[nownodes.nodes[i]] >= pivot)
				s_node.insert(nownodes.nodes[i]);
		}
		nownodes.clear();
		nownodes.insert(s_node);
	}
...
bool Graph::DinicBFS() {
	...
	while (!Q.empty()) {
		...
		for (type i = 0; i < adj_length[u]; i++) {
			...
			if (d[from] < pivot || adj_length[from] / 2 >= volR) {
...
{
	if (d[u] < pivot || adj_length[u] / 2 >= volR) {
		d[u]++; d[e[p[u]].to]--; e[p[u]].to = u;
		if (adj_length[u] / 2 < volR && !allnodes.in[u])
			allnodes.insert(u);
		if (d[u] == pivot && adj_length[u] / 2 < volR)
			nownodes.insert(u);
		return true;
	...
	for (type& i = cur[u]; i < adj_length[u]; i++) {
		...
		if (ne.to != u) continue;
		type from = ne.t1 == u ? ne.t2 : ne.t1;
		if ((dist[from] != dist[u] + 1) && !(d[from] < pivot || adj_length[from] / 2 >= volR)) continue;
		p[from] = adj[u][i];
		if (DinicDFS(from)) {
			if (s_node.in[u]) {
...
void Graph::output() {
	...
	for (type i = 0; i < NearDensest.size; i++) {
		...
		if (!R.in[u]) {
			penalty += adj_length[u] / 2;
			vol_S_cap_nR += adj_length[u] / 2;
		}

		if (R.in[u]) {
			vol_R_cap_S += adj_length[u] / 2;
			S_cap_R++;
		}
	...
	// printf("local vertex ratio: %d / %d = %lf \n", finalnodes.size, n, 1.0 * finalnodes.size/n);  

	// printf("%lf\t", 1.0 * (2 * Ecnt - penalty) / NearDensest.size);
	...
	// printf("%lf\t%lf\n", 1.0 * memory / 1024 / 1024, 1.0 * finalnodes/n);
...
int main(int argc, char** argv) {
	...
	for (type i = 0; i < RSet.size(); i++) 
	{
		...
		// printf("the %d query time is %.6f seconds\n", i, runtime);
		G->output();
		if (G->check() == false) {
